...
{
    private const string TakeTurnErroMessage =
    ...
    private Guid ActiveMemberId =>
        ActiveColor == Domain.ValueObjects.Color.White ? White.MemberId : Black.MemberId;
    ...
    public async Task ShowAvailableMovesAsync(Guid pieceId)
    {
        ...
        if (piece is King kingPiece)
            moves = moves.Concat(GetCastlingMoves(kingPiece));
    ...
    private async Task<TurnResult> ExecuteTurnCommand(PieceEntity activePiece, Square endPosition)
    {
        var command = new TakeTurn()
        {
            ...
            StartPosition = activePiece.Position,
    ...
    private async Task HandleTurnResultAsync(
        TurnResult turnResult,
        PieceEntity activePiece,
        Square endPosition
    )
    {
        ...
        var targetPiece = Pieces.FirstOrDefault(p =>
            p.Position == endPosition && p.Color != activePiece.Color
        );

        activePiece.MoveTo(endPosition);
        ...
        ActiveColor =
            ActiveColor == Domain.ValueObjects.Color.White
                ? Domain.ValueObjects.Color.Black
                : Domain.ValueObjects.Color.White;

        await OnTurnEnded.InvokeAsync(
            new() { ActiveMemberId = ActiveMemberId, Result = turnResult }
        );
    ...
    private IEnumerable<Square> GetCastlingMoves(King king)
    {
        ...
        if (
            !Pieces.Any(p =>
                p.Position == new Square(File.G, rank) || p.Position == new Square(File.F, rank)
            )
        )
        {
    ...
    private async Task<IEnumerable<Square>> GetEnPassantMoves(Pawn pawn)
    {
        ...
        if (!isPawnMove || !isTwoRankMove || !sameRank)
            return Enumerable.Empty<Square>();

        var file = opponentTurn?.StartPosition?.File ?? File.Undefined;
        var rank = rankDiff > 0 ? pawn.Position.Rank + 1 : pawn.Position.Rank - 1;
    ...
    private void HandleCastlingMove(PieceEntity activePiece, CastlingType castlingType)
    {
        ...
        if (rook != null)
        {
            ...
            rook.MoveTo(new(newFile, rook.Position.Rank));
